...
public class CalculerRemunerationServiceSimple implements CalculerRemunerationService {
	public ResultatCalculRemuneration calculer(BulletinSalaire bulletin) {
		...
		for (Cotisation coti : cotisationNonImposable) {
			if (coti.getTauxSalarial() != null) {
				totalRetenueSalariale = totalRetenueSalariale.add(coti.getTauxSalarial().multiply(salaireBrut));
			}
			if (coti.getTauxPatronal() != null) {
				totalCotisationsPatronales = totalCotisationsPatronales
						.add(coti.getTauxPatronal().multiply(salaireBrut));
			}
		}
		...
		for (Cotisation coti : cotisationImposable) {
		...
		res.setTotalRetenueSalarial(totalRetenueSalariale.toString());
		return null;
	}
}
